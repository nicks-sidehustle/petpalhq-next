...
export function buildProductAudit() {
  ...
  const productsMissingAmazonTag = products.filter(
    (product) => product.amazonLink && !/[?&]tag=petpalhq08-20\b/.test(product.amazonLink),
  );
...
export function buildPicksAudit() {
  // Products live in guide frontmatter picks[] (canonical since the frontmatter migration);
  // the flat parseFrontmatter above can't see nested YAML, so gray-matter does the parsing.
  const guides = fs.existsSync(GUIDES_DIR)
    ? fs.readdirSync(GUIDES_DIR).filter((file) => file.endsWith('.md')).sort()
    : [];
  ...
  for (const file of guides) {
    const slug = file.replace(/\.md$/, '');
    try {
      const { data } = matter(readText(path.join(GUIDES_DIR, file)));
      for (const pick of Array.isArray(data.picks) ? data.picks : []) {
        picks.push({ guide: slug, name: pick.name ?? null, asin: pick.asin ?? null, price: pick.price ?? null, image: pick.image ?? null });
      }
    } catch (error) {
      parseFailures.push({ guide: slug, error: error.message });
    }
  }
  ...
  return {
    ...
    parseFailures,
    picksMissingAsin: picks.filter((pick) => !pick.asin),
    picksInvalidAsin: picks.filter((pick) => pick.asin && !asinPattern.test(pick.asin)),
    picksMissingPrice: picks.filter((pick) => !pick.price),
    picksMissingImage: picks.filter((pick) => !pick.image),
    duplicateAsins: duplicates(picks.map((pick) => pick.asin).filter(Boolean)),
  };
}

export function renderGuideAuditMarkdown(audit) {
  return `# PetPalHQ Current-State Audit
...
- Products missing Amazon tag \`petpalhq08-20\`: ${audit.productsMissingAmazonTag.length}
