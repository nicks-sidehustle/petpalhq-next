...
keywords:
  ...
  - "feeding station multi-pet home"
  - "prescription diet feeder food theft"
  - "how to separate pets at feeding time"
  - "slow feeder fast eater multi-pet"
...
picks:
  - rank: 1
    label: "THE ACCESS ANCHOR — MICROCHIP FEEDER"
    name: "Sure Petcare SureFeed Microchip Pet Feeder"
    brand: "Sure Petcare"
    score: 8.6
    price: "$199.00"
    ...
    authoritySources:
      - outlet: "Sure Petcare (Amazon product listing, SureFeed Microchip Pet Feeder)"
        ...
        supports: "spec"
        accessed: "2026-07-16"
      - outlet: "Multi-pet feeding-station consensus (Sure Petcare support guidance and multi-pet households)"
        stat: "a sealed microchip feeder protects one diet per unit, so a home with two protected diets usually needs a second feeder, and stations work best placed apart so a second pet cannot shoulder in over the open bowl"
        supports: "general"
        accessed: "2026-07-16"
    keyFeatures:
  ...
  - rank: 2
    ...
    authoritySources:
      - outlet: "Sure Petcare (Amazon product listing, SureFeed Microchip Pet Feeder Connect)"
        ...
        accessed: "2026-07-16"
      - outlet: "Multi-pet feeding-station consensus (Sure Petcare support guidance and multi-pet households)"
        stat: "per-pet intake logging helps an owner notice when one animal suddenly eats less, which is a welfare signal worth a veterinary call rather than a reading the app itself can diagnose"
        supports: "general"
        accessed: "2026-07-16"
    keyFeatures:
    ...
    verdict: "Choose the Connect when logging who ate is worth the upcharge over plain access control — a home tracking two diets gets real value from the record. Confirm the separately sold hub is in the cart, and treat any intake change as a prompt to call a veterinarian, not a reading to act on alone."

  - rank: 3
    label: "PORTION AND SCHEDULE CONTROL — CAMERA FEEDER"
    name: "PETLIBRO Granary Smart Camera Feeder"
    brand: "PETLIBRO"
    score: 8.4
    price: "$139.99"
    ...
    authoritySources:
      - outlet: "PETLIBRO (Amazon product listing, Granary Smart Camera Feeder)"
        ...
        supports: "spec"
        accessed: "2026-07-16"
      - outlet: "Multi-pet feeding-station consensus (small-frequent-meal feeding guidance)"
        stat: "scheduled portioned meals replace the all-day open bowl that lets one pet graze another's food, and a supervised meal window is far easier to police than free-feeding"
        supports: "general"
        accessed: "2026-07-16"
    keyFeatures:
    ...
    verdict: "Use the Granary to replace free-feeding with supervised, timed meals and to see who is really eating. It is the time lever, not the access lever, so pair it with a sealed feeder if a pet still raids during the window, and confirm current price before buying while the discount stands."

  - rank: 4
    label: "SLOW THE GULPER — ANTI-GULP BOWL"
    name: "Outward Hound Fun Feeder Slo Bowl"
    brand: "Outward Hound"
    score: 8.3
    price: "$8.47"
    ...
    authoritySources:
      - outlet: "Outward Hound (Amazon product listing, Fun Feeder Slo Bowl)"
        ...
        supports: "spec"
        accessed: "2026-07-16"
      - outlet: "Veterinary nutrition guidance (Tufts Petfoodology)"
        stat: "eating too fast raises the risk of choking, vomiting, and swallowed air, and a maze bowl is a low-cost way to slow the gulper so it finishes alongside the slower pet instead of moving on to that bowl"
        supports: "general"
        accessed: "2026-07-16"
    keyFeatures:
    ...
    verdict: "Add a slow bowl when the theft is really a speed gap — the fast eater that finishes first and moves to the other bowl. Match the ridge depth to your pet's muzzle, and remember it fixes timing, not access, so pair it with separation if a raider still finishes early."

  - rank: 5
    label: "HEIGHT SEPARATION — ELEVATED STATION"
    ...
    authoritySources:
      - outlet: "PawHut (Amazon product listing, Elevated Dog Bowls with Storage Cabinet)"
        ...
        supports: "spec"
        accessed: "2026-07-16"
      - outlet: "Veterinary guidance (VCA Animal Hospitals) on raised feeding"
        stat: "whether raised bowls help or harm a large, deep-chested dog is debated as a possible bloat factor, so owners of big breeds should ask a veterinarian before switching to a raised station"
        supports: "general"
        accessed: "2026-07-16"
    keyFeatures:
  ...
  - rank: 6
    label: "WATER WITHOUT GUARDING — MULTI-PET FOUNTAIN"
    ...
    authoritySources:
      - outlet: "PetSafe (Amazon product listing, Drinkwell 360 Multi-Pet Fountain)"
        ...
        supports: "spec"
        accessed: "2026-07-16"
      - outlet: "Feline welfare guidance (ISFM / International Cat Care)"
        stat: "a dominant pet lingering at a single water source can stop a timid one from drinking, so multiple drinking points at one fountain remove the standoff that turns water into a guarded resource"
        supports: "general"
        accessed: "2026-07-16"
    keyFeatures:
    ...
    verdict: "Add the Drinkwell 360 when two pets treat a single water bowl as a resource to guard. The all-around access gives each pet its own drinking point, so neither has to face the other; just plan the weekly clean around its extra parts and measure the spot before it arrives."

comparison:
  rows:
    - label: "Separation axis"
      values: ["Access (microchip)", "Access + logging", "Time / schedule", "Meal rate", "Height", "Water access"]
    - label: "Theft it solves"
      values: ["Diet raiding", "Diet raiding + tracking", "Free-feeding grazing", "Fast-eater bullying", "Cat-dog floor reach", "Fountain guarding"]
    - label: "Cheapest levers first?"
      values: ["Last rung — technology", "Last rung — technology", "Time lever", "Cheapest lever", "Space / height lever", "Space lever"]
...
methodology:
  ...
  factors:
    - name: "Expert Consensus"
      weight: 35
      definition: "Synthesized from Sure Petcare support guidance on feeder food-stealing, manufacturer documentation, and veterinary and feline-welfare references (Tufts Petfoodology, VCA Animal Hospitals, ISFM / International Cat Care). The PetPal Feeding-Station Score is a composite of expert opinion — PetPalHQ does not run a testing lab."
    - name: "Problem-Fit"
      weight: 25
      definition: "How directly the item closes a specific food-theft access gap — a raided prescription diet, an all-day open bowl, a fast eater that moves to the other bowl, a floor-level raid, or a guarded water source — rather than how it performs as a standalone product."
    - name: "Multi-Pet Logistics Design"
      weight: 20
      definition: "How well the item supports a correct separation plan: controlling who reaches which bowl, placing stations apart, and working the cheap levers of time and space before the expensive lever of technology, so access is controlled rather than pets retrained."
    - name: "Value"
      weight: 20
...
Food stealing in a multi-pet home looks like a discipline problem, and it almost never is. Someone with a cat and a dog, or two cats on different diets, watches one animal clean out the other's bowl and assumes the fix is teaching the pets to leave each other's food alone. That training rarely takes, because the behavior is not defiance — it is opportunity. The food is reachable and the mealtime is unsupervised, and any animal will take a free meal that is sitting there. Control who can reach which food, and the problem disappears without a single training session.
