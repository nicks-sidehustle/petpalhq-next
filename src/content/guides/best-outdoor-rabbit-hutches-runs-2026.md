---
title: "Best Outdoor Rabbit Hutches & Runs 2026: Predator-Proofing, Weatherproofing, and How Much Space a Rabbit Really Needs"
description: "Five outdoor rabbit hutches you can actually buy on Amazon, judged honestly on predator security, weatherproofing, and usable run space. The uncomfortable truth first: none of these meets the RWAF minimum footprint on its own, and the wire these makers do document — COZIWOW's galvanized mesh, Rockever's plated iron — is not the 1/2-inch hardware cloth a fox or raccoon cannot beat. Here is how to use them safely anyway."
excerpt: "The outdoor hutches worth buying — and the honest gap between a store-bought hutch and the space, mesh, and predator-proofing a rabbit actually needs to be safe outside."
category: "Playground"
keywords:
  - "outdoor rabbit hutch"
  - "rabbit hutch with run"
  - "predator proof rabbit hutch"
  - "weatherproof rabbit hutch"
  - "large rabbit hutch outdoor"
  - "2 story rabbit hutch"
  - "rabbit run space requirements"
guideType: "spoke"
publishDate: "2026-07-05"
updatedDate: "2026-07-05"
readTime: "12 min"
featured: false
heroImage: "/images/guides/best-outdoor-rabbit-hutches-runs-2026.webp"
products: []
reviewMethod: "Editorial synthesis of rabbit-welfare guidance and manufacturer listing specifications. Space and housing standards are drawn from the Rabbit Welfare Association & Fund (RWAF) minimum-space recommendations and its 'Reality of Living in a Hutch' guidance, predator and mesh guidance from The Rabbit House and the RSPCA Knowledgebase, and per-product specifications from each maker's verified Amazon listing (Ketive, Rockever, mayugardening, GUTINNEEN, COZIWOW), confirmed through Amazon's Creators API on 2026-07-05. PetPalHQ does not run a materials or predator-resistance testing lab; the scores below are a synthesis of welfare guidance and documented specifications, not a measurement."
lastProductCheck: "2026-07-05"
expertSourceCount: 7

shortAnswer: "Start here: a store-bought hutch is a shelter, not a home. The RWAF sets a minimum footprint of 3m x 2m x 1m (about 10 by 6 by 3 feet) for a pair of rabbits, available at all times, and none of these five hutches comes close on its own — the largest, the COZIWOW 92-inch, is still under eight feet long. Nor should you trust any of them on their factory wire: the half-inch hardware cloth that actually stops a fox or raccoon is a standard the documented wire here — COZIWOW's galvanized mesh, Rockever's plated iron — does not meet. Buy one as secure sleeping-and-shelter housing attached to a much larger run, and reinforce the mesh and latches. On that basis: the GUTINNEEN metal-frame 62-inch hutch (about $170) is the sturdiest and most chew-resistant; the COZIWOW 92-inch (about $161) gives the most run space; the Ketive 48-inch (about $80) is the best value two-story; the mayugardening 36-inch (about $80) is the smallest, most mobile starter; and the Rockever hutch-with-run (about $130) is a simple low enclosure best for supervised daytime use."

topPicks:
  - name: "GUTINNEEN 2-Story Rabbit Hutch, Chew-Proof Full Metal Frame (62 inch)"
    pickRef: "r1"
    keyFeature: "The sturdiest build here: a solid-wood body on an aluminum-alloy reinforced frame that resists chewing and warping, with 10.93 square feet of living space across two levels, three lockable pull-out trays, and six casters (two braked). It is the closest thing in this roundup to a hutch that survives an active rabbit and a wet winter — but plan on adding hardware cloth and latch locks for predator country."
    sources: ["GUTINNEEN Amazon listing (verified specs)", "RWAF minimum-space guidance", "The Rabbit House — wire mesh guidance"]
    verifiedDate: "2026-07-05"
  - name: "COZIWOW Large 92-inch Outdoor Rabbit Hutch with 3 Run Boxes"
    pickRef: "r2"
    keyFeature: "The most run space in the group: a 92-inch-long weatherproof hutch with two large side runs, an elevated shelter, a 0.5-inch asphalt roof, and an open-bottom design that lets rabbits graze on grass. The open bottom is the catch — it is a dig-in route for predators, so it needs a paved base or a buried skirt of hardware cloth to be safe."
    sources: ["COZIWOW Amazon listing (verified specs)", "RSPCA — where to keep rabbits", "RWAF — reality of living in a hutch"]
    verifiedDate: "2026-07-05"
  - name: "Ketive 48-inch 2-Story Weatherproof Rabbit Hutch"
    pickRef: "r3"
    keyFeature: "The best-value two-story hutch: solid fir wood with a UV-resistant panel and asphalt roof, a 2.45-square-foot upper penthouse over a 5.66-square-foot lower run, four access doors, and a deep slide-out tray, for about $80. A capable shelter for the money, provided you treat it as sleeping quarters attached to a larger run rather than a full home."
    sources: ["Ketive Amazon listing (verified specs)", "RWAF minimum-space guidance"]
    verifiedDate: "2026-07-05"

picks:
  - rank: 1
    label: "BEST OVERALL — STURDIEST, MOST CHEW-RESISTANT"
    name: "GUTINNEEN 2-Story Rabbit Hutch with Chew-Proof Full Metal Frame for Indoor/Outdoor, 62 inch"
    brand: "GUTINNEEN"
    score: 8.7
    price: "$169.99"
    image: "https://m.media-amazon.com/images/I/61HrP4RORPL._SL500_.jpg"
    asin: "B0F8MVLP8Z"
    aliases:
      - "GUTINNEEN 62-inch hutch"
      - "the GUTINNEEN"
      - "GUTINNEEN metal-frame hutch"
    keyFeatures:
      - "62 in L x 22.6 in W x 33.5 in H with 10.93 square feet of living space across two levels"
      - "Solid wood on an aluminum-alloy reinforced frame the maker rates as chew-proof and warp-resistant"
      - "Three lockable deep pull-out trays plus five access doors and an openable run roof for cleaning"
      - "Six swivel casters, two with safety brakes, to move it between patio and yard"
      - "Check the wire gauge and gap on arrival and reinforce with half-inch hardware cloth for real predator country"
    body: |
      The GUTINNEEN 62-inch hutch earns the top spot because it fixes the failure that kills most wooden hutches: it does not fall apart. The body is solid wood, but it sits on an aluminum-alloy reinforced frame that the maker rates as chew-proof and resistant to the warping and cracking that ruin cheaper all-wood boxes after a winter or two. At 62 inches long with 10.93 square feet of living space over two levels, it is roomy for a single rabbit and workable for a bonded pair as their sleeping-and-shelter unit — though, as with every hutch here, a pair still needs a much larger attached run to meet real welfare standards. If you want to compare current pricing on this class of build, you can [browse chew-proof metal-frame rabbit hutches on Amazon](https://www.amazon.com/s?k=chew+proof+metal+frame+rabbit+hutch&tag=petpalhq08-20) before you commit.

      Daily upkeep is where it pulls ahead. Three lockable deep pull-out trays mean you can clean one level without disturbing the other, five access doors plus an openable run roof kill the awkward reaching that makes hutch-cleaning a chore, and the six casters (two braked) let you roll it into shade or out of a storm. Those are the details that decide whether you actually keep the enclosure clean, and a clean hutch is the single biggest health lever you control.

      Here is the honest limit. The GUTINNEEN metal-frame hutch is sturdier than its rivals, but the frame is the part that is upgraded, not the wire. Half-inch hardware cloth is the standard a determined fox or raccoon cannot defeat, and its slide latches are simple enough for a clever raccoon to work. In a yard with real predator pressure, you should reinforce the panels with half-inch hardware cloth and add carabiners or padlocks to the latches. And at about $170 it costs roughly twice the budget picks — you are paying for the frame, so only buy it if durability is the feature you actually need.
    pros:
      - "Aluminum-alloy reinforced frame resists chewing and warping better than any all-wood hutch here"
      - "10.93 square feet over two levels — generous for a single rabbit's shelter unit"
      - "Three lockable pull-out trays, five doors, and an openable roof make cleaning genuinely easy"
      - "Six casters (two braked) for moving it out of weather or into shade"
    cons:
      - "Panels need half-inch hardware cloth added for predator country"
      - "Simple slide latches a raccoon can defeat; add carabiners or padlocks"
      - "About twice the price of the budget two-story hutches"
      - "Still a shelter, not primary housing — a pair needs a much larger attached run"
    verdict: "If you want the hutch least likely to be chewed apart or warped by a wet winter, the GUTINNEEN is the pick. Budget for hardware cloth and latch locks, and treat it as secure sleeping quarters attached to a bigger run — not the whole home."
    authoritySources:
      - outlet: "GUTINNEEN (Amazon listing, verified specs)"
        url: "https://www.amazon.com/dp/B0F8MVLP8Z"
        stat: "62 in x 22.6 in x 33.5 in with 10.93 sq ft of space, solid wood on an aluminum-alloy reinforced chew-proof frame, three lockable pull-out trays, five doors, and six casters (two braked)"
        claim: "10.93 sq ft two-level hutch on a chew-proof reinforced frame with three pull-out trays and six casters"
        supports: "spec"
        accessed: "2026-07-05"
      - outlet: "The Rabbit House (wire mesh guidance)"
        url: "https://www.therabbithouse.com/outdoor/rabbit-mesh.asp"
        stat: "welded mesh is strongest for rabbit housing; maximum safe gap is 1 inch, with 1/2 inch giving real predator security — chicken wire is inadequate against predators"
        claim: "Half-inch welded hardware cloth is the predator-resistant standard; standard galvanized mesh is weaker"
        supports: "general"
        accessed: "2026-07-05"

  - rank: 2
    label: "BEST FOR RUN SPACE — LONGEST HUTCH"
    name: "COZIWOW Large 92-inch Outdoor Rabbit Hutch with 3 Run Boxes, Weatherproof, Asphalt Roof"
    brand: "COZIWOW"
    score: 8.5
    price: "$161.49"
    image: "https://m.media-amazon.com/images/I/6164g38NMrL._SL500_.jpg"
    asin: "B0GT4KJBNP"
    aliases:
      - "COZIWOW 92-inch hutch"
      - "the COZIWOW"
      - "COZIWOW 3-run hutch"
    keyFeatures:
      - "92 inches long with two large side runs and an elevated shelter, sized for two to three small-to-medium rabbits"
      - "0.5-inch asphalt roof (waterproof and UV-resistant) over a solid fir-wood frame with galvanized wire mesh"
      - "Open-bottom run design so rabbits can graze directly on grass"
      - "Hands-free pull-out tray under the rest area, separated by wire mesh from the living space"
      - "Back-panel ventilation hole plus a cable pass-through for a winter heating pad"
    body: |
      The COZIWOW 92-inch hutch wins on the one thing rabbits need most and hutches skimp on hardest: length to move. At 92 inches with two side runs flanking a raised shelter, it gives a rabbit room to take a few real hops instead of a single shuffle, which is exactly the behavior the welfare guidance cares about. The open-bottom run lets your rabbits graze on grass, satisfying a natural foraging drive, and the 0.5-inch asphalt roof and fir-wood frame hold up to weather better than a flat-panel lid. For a pair that gets supervised lawn time, this is the roomiest ready-made option on Amazon; you can [see long outdoor rabbit hutches with runs](https://www.amazon.com/s?k=large+outdoor+rabbit+hutch+with+run&tag=petpalhq08-20) to sanity-check the footprint against your space.

      The maintenance thinking is good, too. A hands-free pull-out tray sits under the covered rest area, separated from the rabbits by wire mesh so they stay out of their own waste, and a back-panel cable pass-through lets you run a winter heating pad or a summer fan without leaving a door cracked. A dedicated ventilation hole keeps the enclosed shelter from turning stuffy in heat.

      Now the trade-off that comes with that grazing floor: the open bottom is a two-way door. It is great for foraging and terrible for security, because a fox or dog can dig straight under the run, and a rabbit can dig its way out. Do not set this hutch on bare grass in predator country without a defense — either place it on a paved or slabbed base, or bury a skirt of half-inch hardware cloth around and under the perimeter. The galvanized mesh panels are not half-inch hardware cloth either, so the same reinforcement logic applies to the sides. Treated as a daytime grazing run with a secured base, the COZIWOW is excellent; left open on a lawn overnight, it is a risk.
    pros:
      - "92 inches long — the most room to hop of any hutch here, for two to three rabbits"
      - "0.5-inch asphalt roof and fir frame handle weather better than flat-lid boxes"
      - "Open-bottom run enables natural grazing on grass"
      - "Hands-free waste tray and a cable pass-through for seasonal heating or cooling"
    cons:
      - "Open bottom is a dig-in predator route and a dig-out escape route without a secured base"
      - "Galvanized mesh, not half-inch hardware cloth — reinforce the panels in predator country"
      - "Long footprint needs real yard space and a level spot"
      - "Still short of the RWAF single-block run size for a pair on its own"
    verdict: "If usable hopping room is your priority and you can put it on a paved base or bury a hardware-cloth skirt, the COZIWOW gives the most space here. Never leave the open bottom unsecured on a lawn overnight."
    authoritySources:
      - outlet: "COZIWOW (Amazon listing, verified specs)"
        url: "https://www.amazon.com/dp/B0GT4KJBNP"
        stat: "92 inches long with two side runs and an elevated shelter for 2-3 rabbits, a 0.5-inch waterproof/UV asphalt roof, fir-wood frame with galvanized wire mesh, open-bottom grazing design, hands-free tray, and a back-panel cable pass-through"
        claim: "92-inch weatherproof hutch with two runs, open grazing bottom, and a hands-free waste tray"
        supports: "spec"
        accessed: "2026-07-05"
      - outlet: "RSPCA Knowledgebase (where to keep rabbits)"
        url: "https://kb.rspca.org.au/categories/companion-animals/rabbits/where-should-i-keep-my-rabbits"
        stat: "outdoor rabbits must be protected from predators, and an enclosure needs a single block of space large enough for rabbits to run, not just hop"
        claim: "Outdoor rabbits need predator protection and a single connected block of run space"
        supports: "general"
        accessed: "2026-07-05"

  - rank: 3
    label: "BEST VALUE — TWO-STORY UNDER $100"
    name: "Ketive 48-inch 2-Story Outdoor/Indoor Rabbit Hutch, Weatherproof with Deep Tray"
    brand: "ketive"
    score: 8.3
    price: "$79.99"
    image: "https://m.media-amazon.com/images/I/51AjTXOGgEL._SL500_.jpg"
    asin: "B0FDQJNG59"
    aliases:
      - "Ketive 48-inch hutch"
      - "the Ketive"
      - "Ketive two-story hutch"
    keyFeatures:
      - "48 in L x 23.6 in W x 36.2 in H with a 2.45-square-foot upper penthouse over a 5.66-square-foot lower run"
      - "Solid fir wood with a UV-resistant panel and an asphalt roof for year-round use"
      - "Four access doors plus a deeper slide-out tray for easy cleaning"
      - "Combines with a second unit to expand the footprint"
      - "Maker rates it for 1-3 rabbits — optimistic against real single-block space needs"
    body: |
      The Ketive 48-inch hutch is the value pick because it covers the basics for about $80 without cutting the parts that matter for a shelter. It is solid fir wood with a UV-resistant panel and an asphalt roof, so it is genuinely built for weather rather than just splash-resistant, and the two-story layout stacks a cozy 2.45-square-foot upper penthouse over a 5.66-square-foot lower run to squeeze useful space out of a small footprint. Four access doors and a deep slide-out tray make daily cleaning quick. If your budget tops out under three figures, the Ketive is the most house you can buy for the money; it is worth [checking budget two-story rabbit hutches](https://www.amazon.com/s?k=2+story+outdoor+rabbit+hutch&tag=petpalhq08-20) to see how thin most rivals get at this price before you decide.

      The expandability is a real plus. Because you can bolt a second Ketive unit onto the first, you can grow the enclosure as your setup matures instead of buying a whole new hutch — a sensible path from a starter shelter toward something closer to adequate run space.

      The honest caveats are the ones that apply to every wooden hutch, only more so at this price. The maker's 1-to-3-rabbit rating is optimistic: 8 square feet total is fine as a single rabbit's shelter but well short of the space a pair needs to actually live well, so treat the number as a maximum-occupancy sticker, not a welfare target. The fir wood and asphalt roof will still need reseal and inspection to fight rot at the joints over a few wet seasons, and the mesh and latches are budget-grade, so the same hardware-cloth-and-carabiner reinforcement applies. As a weatherproof shelter attached to a larger run, though, the Ketive punches above its price.
    pros:
      - "About $80 for a genuinely weatherproof solid-fir, asphalt-roof two-story hutch"
      - "Two levels put 8 square feet of usable space in a compact footprint"
      - "Four doors and a deep slide-out tray make cleaning fast"
      - "Expands by joining a second identical unit"
    cons:
      - "Maker's 1-3 rabbit rating overstates real capacity for a pair"
      - "Budget wood and roof need resealing to resist rot over wet seasons"
      - "Budget mesh and latches — reinforce with hardware cloth and locks for predators"
      - "A shelter, not a full home; pair it with a larger run"
    verdict: "For the best weatherproof hutch under $100, the Ketive is the pick. Read its capacity rating as a ceiling, not a goal, and plan to attach a larger run for anything more than one rabbit."
    authoritySources:
      - outlet: "Ketive (Amazon listing, verified specs)"
        url: "https://www.amazon.com/dp/B0FDQJNG59"
        stat: "48 in x 23.6 in x 36.2 in, upper penthouse 2.45 sq ft over a lower run of 5.66 sq ft, solid fir wood with a UV-resistant panel and asphalt roof, four access doors, a deep slide-out tray, and two-unit expandability"
        claim: "Weatherproof solid-fir two-story hutch with ~8 sq ft total space and expandability, around $80"
        supports: "spec"
        accessed: "2026-07-05"
      - outlet: "RWAF (minimum space requirements)"
        url: "https://rabbitwelfare.co.uk/space-recommendations/"
        stat: "recommends a minimum footprint of 3m x 2m x 1m (about 10 x 6 x 3 feet) for a pair of rabbits, available at all times, with a hutch acting only as a shelter"
        claim: "A pair of rabbits needs a 3m x 2m x 1m single-block footprint; a hutch is a shelter, not the whole home"
        supports: "general"
        accessed: "2026-07-05"

  - rank: 4
    label: "BEST COMPACT & MOBILE STARTER"
    name: "mayugardening 2-Story Rabbit Hutch, 36 inch, 4 Wheels, Leak-Proof Trays"
    brand: "mayugardening"
    score: 7.9
    price: "$79.99"
    image: "https://m.media-amazon.com/images/I/51+lyjqe7WL._SL500_.jpg"
    asin: "B0G25GNR4Z"
    aliases:
      - "mayugardening hutch"
      - "the mayugardening"
      - "mayugardening 36-inch hutch"
    keyFeatures:
      - "36 in L x 17.8 in W x 36.7 in H with a 3.98-square-foot main level and a 1.8-square-foot upper level"
      - "Solid cedar wood with a waterproof asphalt roof"
      - "Two leak-proof slide-out trays and a top-lift metal feeder"
      - "Four locking wheels for easy repositioning"
      - "Chew-resistant metal bars reinforce the run against splitting"
    body: |
      The mayugardening 36-inch hutch is the compact, movable option for a small yard, balcony, or shed corner where the bigger hutches simply will not fit. It is solid cedar with a waterproof asphalt roof, and its four locking wheels are the standout feature: you can roll it into shade during a heat wave, out of a downpour, or across the patio for cleaning without dismantling anything. Cedar also has a natural rot-resistance edge over plain fir, which helps in damp spots. For tight spaces, it is a sensible starter — compare it against other [compact rabbit hutches with wheels](https://www.amazon.com/s?k=small+rabbit+hutch+with+wheels&tag=petpalhq08-20) to confirm the dimensions suit your spot.

      The build has thoughtful touches for the size. Two leak-proof slide-out trays keep cleaning tidy across both levels, a top-lift metal feeder lets you top up food without opening a door, and chew-resistant metal bars reinforce the run so an active rabbit cannot split the wood as quickly.

      The trade-off is blunt: this is the smallest hutch here, and small is the thing rabbits suffer from most. With roughly 5.8 square feet total, the mayugardening is strictly a shelter or a very short-term starter, never a place a rabbit should spend most of its day. Its whole value proposition — compactness and mobility — is also its welfare ceiling, so pair it with the largest run you can give and use the wheels to maximize that run's daylight access. As with the others, the mesh and latches are budget-grade and want reinforcement in predator country.
    pros:
      - "Four locking wheels make it genuinely easy to move out of weather or for cleaning"
      - "Solid cedar with a natural rot-resistance edge and a waterproof asphalt roof"
      - "Two leak-proof trays and a top-lift feeder for tidy daily upkeep"
      - "Chew-resistant metal bars protect the run from splitting"
    cons:
      - "Smallest footprint here — about 5.8 square feet total, strictly a shelter"
      - "Not suitable as anywhere a rabbit spends most of its day"
      - "Budget mesh and latches need hardware-cloth and lock reinforcement"
      - "Two rabbits would quickly outgrow it"
    verdict: "For a small space or a mobile setup, the mayugardening is a fine compact starter — as long as you treat it as a movable shelter attached to a much larger run, not a home."
    authoritySources:
      - outlet: "mayugardening (Amazon listing, verified specs)"
        url: "https://www.amazon.com/dp/B0G25GNR4Z"
        stat: "36 in x 17.8 in x 36.7 in, a 3.98 sq ft main level plus a 1.8 sq ft upper level, solid cedar with a waterproof asphalt roof, two leak-proof pull-out trays, a top-lift feeder, four locking wheels, and chew-resistant metal bars"
        claim: "Compact ~5.8 sq ft two-story cedar hutch on four locking wheels with two trays"
        supports: "spec"
        accessed: "2026-07-05"

  - rank: 5
    label: "BEST SIMPLE DAYTIME ENCLOSURE"
    name: "Rockever Wooden Hutch with Run, Indoor/Outdoor Small-Animal Enclosure"
    brand: "Rockever"
    score: 7.6
    price: "$129.99"
    image: "https://m.media-amazon.com/images/I/61xJHguQlCL._SL500_.jpg"
    asin: "B07V56SYXP"
    aliases:
      - "Rockever hutch"
      - "the Rockever"
      - "Rockever hutch with run"
    keyFeatures:
      - "Outer footprint about 49 in x 36 in x 14 in, with a 13 in x 36 in x 14 in grated run area"
      - "Solid wood body with a plated-iron wire enclosure for ventilation and viewing"
      - "Removable bottom floor in the sheltered hut, no bottom in the run for grazing"
      - "Marketed generically for guinea pigs, rabbits, chicks, and tortoises"
      - "Low 14-inch height limits vertical room and standing space"
    body: |
      The Rockever hutch with run is the simple, low-profile enclosure in this group — a wide, single-level footprint rather than a tall two-story box. Its strength is a big grated run area (about 13 by 36 inches) attached to a covered wooden hut, with a plated-iron wire enclosure that gives good ventilation and clear sight lines to your rabbit. The sheltered hut has a removable bottom floor for easy cleaning, while the run has no floor so your rabbit can nibble grass. For a supervised daytime pen on a lawn, it does a straightforward job, and it is worth [comparing simple rabbit hutch-and-run enclosures](https://www.amazon.com/s?k=rabbit+hutch+with+run+enclosure&tag=petpalhq08-20) at this price point.

      Two honest limits keep it at the bottom of the ranking, and neither is a defect so much as a boundary. First, it is low: at roughly 14 inches tall, the run gives far less vertical room than the two-story hutches, so a full-grown rabbit cannot stand up fully or take a real leap, and the space suits a smaller rabbit or a guinea pig better than a large breed. Second, the listing markets it generically across guinea pigs, rabbits, chicks, and tortoises, which is a sign it is a general small-animal enclosure rather than a rabbit-specific design — fine for what it is, but not optimized for a rabbit's need to stretch and hop.

      The predator math is the same as the rest, with the open-run floor adding a dig risk. The plated-iron wire is better than chicken wire but still not half-inch hardware cloth, and the run's missing floor invites digging, so use it as a supervised daytime enclosure or set it on a secured base — not as an unattended overnight home.
    pros:
      - "Large grated run area with excellent ventilation and clear viewing"
      - "Removable hut floor for easy cleaning; open run floor for grazing"
      - "Simple, wide single-level layout that is easy to place and access"
      - "Plated-iron wire is sturdier than chicken wire"
    cons:
      - "Only about 14 inches tall — too low for a full-grown rabbit to stand or leap"
      - "Marketed generically across species rather than designed for rabbits"
      - "Open run floor is a dig-in and dig-out risk without a secured base"
      - "Plated-iron wire still is not half-inch hardware cloth for serious predators"
    verdict: "As a simple supervised daytime enclosure for a smaller rabbit or guinea pig, the Rockever works. Its low height and generic design keep it last; do not use it as an unattended overnight rabbit home."
    authoritySources:
      - outlet: "Rockever (Amazon listing, verified specs)"
        url: "https://www.amazon.com/dp/B07V56SYXP"
        stat: "outer size about 49 in x 36 in x 14 in with a 13 in x 36 in x 14 in grated run, solid wood body, plated-iron wire enclosure, a removable floor in the hut and no floor in the run, marketed for guinea pigs, rabbits, chicks, and tortoises"
        claim: "Low single-level wooden hutch-and-run with a grazing floor, generic small-animal design"
        supports: "spec"
        accessed: "2026-07-05"

comparison:
  headers: ["Product", "Price", "Usable space", "Run / floor", "PetPal Hutch Score"]
  rows:
    - ["GUTINNEEN 62-inch metal-frame", "$169.99", "10.93 sq ft, 2 levels", "Enclosed, trays", "8.7"]
    - ["COZIWOW 92-inch, 3 runs", "$161.49", "92 in long, 2 runs", "Open-bottom grazing", "8.5"]
    - ["Ketive 48-inch 2-story", "$79.99", "~8 sq ft, 2 levels", "Enclosed lower run", "8.3"]
    - ["mayugardening 36-inch, wheels", "$79.99", "~5.8 sq ft, 2 levels", "Enclosed, mobile", "7.9"]
    - ["Rockever hutch with run", "$129.99", "Low, ~14 in tall", "Open run floor", "7.6"]

methodology:
  formula: "PetPal Hutch Score = (Predator & Escape Security × 0.25) + (Usable Space & Run × 0.25) + (Weatherproofing & Durability × 0.20) + (Cleaning & Maintenance × 0.15) + (Value × 0.15)"
  factors:
    - name: "Predator & Escape Security"
      weight: 25
      definition: "How well the enclosure resists a determined predator and a digging rabbit out of the box. No hutch here scores full marks: half-inch hardware cloth is the wire welfare and rabbitry guidance treats as the predator-resistant standard, and the wire documented here does not meet it — COZIWOW lists galvanized mesh, Rockever plated iron — while none of the five has a fully dig-proof base. The GUTINNEEN's reinforced frame and enclosed trays rate highest; the open-bottom COZIWOW and floorless Rockever run rate lowest until you add a secured base. This factor rewards the design you start from, not the reinforcement you can add."
    - name: "Usable Space & Run"
      weight: 25
      definition: "How much genuinely usable, hop-capable space the rabbit gets, measured against the RWAF standard that a pair needs a 3m x 2m x 1m single block available at all times. Every hutch here falls short of that on its own, so this factor grades them relative to each other: the 92-inch COZIWOW and the 10.93-square-foot GUTINNEEN lead, the two-story Ketive is respectable for its footprint, and the small mayugardening and low-slung Rockever trail. A high score never means the hutch is enough alone — it means it is the better shelter to attach to a larger run."
    - name: "Weatherproofing & Durability"
      weight: 20
      definition: "How well the hutch survives sun, rain, and an active chewer across seasons — roof quality, wood type, and frame construction. Asphalt roofs and UV panels (Ketive, COZIWOW), cedar's natural rot resistance (mayugardening), and the GUTINNEEN's aluminum-alloy reinforced chew-proof frame all count here. All-wood hutches lose points for the reseal-and-inspect maintenance they need to fight rot at the joints, which is a real recurring cost."
    - name: "Cleaning & Maintenance"
      weight: 15
      definition: "How easy the enclosure is to keep clean, because a clean hutch is the biggest day-to-day health lever an owner controls. Deep pull-out trays, multiple access doors, openable roofs, and hands-free waste separation (the GUTINNEEN's three trays and five doors, the COZIWOW's hands-free tray) score highest. Designs that force awkward reaching or make you disturb the rabbit to clean score lower."
    - name: "Value"
      weight: 15
      definition: "Price relative to what you actually get, judged within the buyer's need rather than as the lowest number. The Ketive scores highest for delivering a genuinely weatherproof two-story hutch under $100; the GUTINNEEN costs the most but justifies it with the frame; the Rockever's mid price for a low, generic enclosure scores lowest. Value here always assumes you will add reinforcement and a larger run, because none of these is a complete solution by itself."

bottomLine:
  - "Buy the GUTINNEEN 62-inch metal-frame hutch if durability is your priority — its reinforced, chew-proof frame and easy-clean trays make it the sturdiest shelter here. Budget for hardware cloth and latch locks, and attach a larger run."
  - "Buy the COZIWOW 92-inch if you want the most hopping room and can secure the open bottom with a paved base or a buried hardware-cloth skirt. Never leave the open floor unsecured on a lawn overnight."
  - "Buy the Ketive 48-inch if you want the best weatherproof hutch under $100. Read its 1-3 rabbit rating as a ceiling, not a goal, and plan a larger attached run for a pair."
  - "Buy the mayugardening 36-inch if space is tight and mobility matters — its locking wheels are genuinely useful. It is a compact shelter only; give it the biggest run you can and use the wheels to chase shade."
  - "Skip all of these as a rabbit's sole home. None meets the RWAF minimum footprint alone, and none should be trusted on its factory wire in predator country. If you cannot provide a large, predator-secured, single-block run in addition to the hutch, keep your rabbits indoors instead."

whenNotToBuy: |
  Do not buy any of these hutches as your rabbit's only home. The RWAF sets a minimum footprint of 3m x 2m x 1m — roughly 10 by 6 by 3 feet — for a pair of rabbits, available at all times, and it is explicit that a hutch should only ever be a shelter, never the sole or main accommodation. A rabbit confined to a hutch alone can develop skeletal problems, sores, and obesity. Every hutch on this page is a sleeping-and-shelter unit that must attach to a much larger secure run. If you cannot provide that run, do not house rabbits outdoors.

  Do not rely on the factory mesh in real predator country. Rabbitry and welfare guidance treat half-inch hardware cloth as the predator-resistant standard, and the wire documented on these hutches — COZIWOW's galvanized mesh, Rockever's plated iron — is not it. A fox, dog, or raccoon can force or tear thin mesh, and a raccoon can reach through a 1-inch gap. If foxes, coyotes, dogs, raccoons, or birds of prey are in your area, reinforce every panel with half-inch hardware cloth before you trust the hutch overnight.

  Do not trust the factory latches against raccoons. Simple slide bolts and turn latches are the kind a clever raccoon learns to open. Add carabiners, spring clips, or small padlocks to every door before leaving rabbits unattended outside.

  Do not set an open-bottom hutch on bare grass without a dig defense. The COZIWOW's grazing floor and the Rockever's floorless run are wonderful for foraging and dangerous for security — a predator can dig under and a rabbit can dig out. Place the hutch on a paved or slabbed base, or bury a skirt of hardware cloth around and under the perimeter.

  Do not skip weather maintenance on all-wood hutches. Fir and cedar with asphalt roofs still need periodic resealing and joint inspection, or they will rot, mold, and let water in over a few wet seasons. If you cannot commit to that upkeep, an outdoor wooden hutch is the wrong choice.

  Reconsider outdoor housing entirely in extreme climates or if predators are heavy. In areas with severe heat, hard freezes, or dense predator pressure, even a reinforced hutch can be unsafe. Many owners in those conditions house rabbits indoors with a large exercise pen instead, which is often the safer and kinder answer.

sources:
  expert:
    - "RWAF (Rabbit Welfare Association & Fund) — Minimum Space Requirements for Pet Rabbits (3m x 2m x 1m footprint for a pair, at all times)"
    - "RWAF — The Reality of Living in a Hutch (a hutch is only ever a shelter)"
    - "The Rabbit House — Wire Mesh for Rabbit Hutches, Cages & Runs (welded mesh, 1-inch max gap, 1/2-inch for predator security)"
    - "RSPCA Knowledgebase — Where should I keep my rabbits (predator protection and single-block run space)"
    - "House Rabbit Society — outdoor housing and predator-risk guidance"
    - "The Spruce Pets — outdoor rabbit hutch buying considerations"
    - "Manufacturer Amazon listings (Ketive, Rockever, mayugardening, GUTINNEEN, COZIWOW) — feature specifications verified via Amazon Creators API, 2026-07-05"
  community:
    - "BackYard Chickens forum — hardware cloth vs. rabbit cage wire threads (owner predator-proofing experience)"
    - "Rabbits Online — recommended housing sizes discussion"
    - "r/Rabbits — outdoor hutch and predator-proofing owner reports"
  verifiedDate: "2026-07-05"
  authorBio: "Nick Miles is the chief editor of PetPalHQ. These picks are an editorial synthesis of rabbit-welfare guidance from the RWAF and RSPCA, predator and mesh guidance from The Rabbit House, and first-party manufacturer specifications verified on Amazon. PetPalHQ does not run a materials or predator-resistance testing lab. The PetPal Hutch Score is a composite of welfare guidance and documented design factors, not a measurement."

ownerVoice: []

related:
  - "best-ferret-cages-multi-level-2026"
  - "best-hamster-habitats-cages-2026"
  - "best-guinea-pig-cages-habitats-2026"
---

Outdoor rabbit hutches are sold on a comforting fiction: that a wooden box with a wire run is a home you can set on the lawn and forget. It is not, and the most useful thing this guide can do is say so before you spend a dollar. A hutch is a shelter — a place to sleep, hide, and get out of the weather — and rabbit-welfare authorities are blunt that it should never be the whole of a rabbit's world. Buy one of these five with that framing and you will keep your rabbits safe and happy; buy one believing the product photos, and you will not.

We read 7 sources for this guide: the Rabbit Welfare Association & Fund's minimum-space recommendations and its "Reality of Living in a Hutch" guidance, mesh and predator guidance from The Rabbit House and the RSPCA Knowledgebase, general outdoor-housing guidance from the House Rabbit Society and The Spruce Pets, and the verified manufacturer specifications for all five hutches, confirmed through Amazon's Creators API on 2026-07-05. PetPalHQ does not run a materials-testing lab; the scores here are a synthesis of welfare guidance and documented specs, not a measurement.

PetPalHQ.com earns affiliate commissions from qualifying purchases. Our recommendations are based on expert consensus — we synthesize professional reviews and manufacturer documentation, plus multi-year owner durability data, to find pet gear that is worth your investment. Commission rates never influence our editorial recommendations.

Two numbers should shape every choice below. The first is the RWAF minimum: a pair of rabbits needs a single block of space measuring about 10 by 6 by 3 feet, available all the time, not just for an hour of exercise. The second is the mesh standard: half-inch welded hardware cloth is what stops a predator, and chicken wire or thin galvanized mesh is not. Hold those two facts up against any hutch — including the one ranked first below — and you will see why we treat every one of these as a shelter to reinforce and attach to a run, rather than a finished solution.

## Why a Store-Bought Hutch Is a Shelter, Not a Home

The gap between what a hutch is and what a rabbit needs is not a rounding error — it is roughly an order of magnitude. The RWAF's minimum footprint of 3m x 2m x 1m works out to about 65 square feet of connected floor space, and it exists because rabbits need room to run and take several hops in a line, not just turn around. The largest hutch here, the COZIWOW 92-inch, is under 8 feet long. Even our roomiest pick provides a fraction of the run a bonded pair should have. That is not a knock on these products; it is the nature of the category. A hutch answers "where does my rabbit sleep and shelter?" and a run answers "where does my rabbit live and move?" — and you need both.

The practical version of this is simple. Attach any of these hutches to a large exercise run — a puppy-pen ring on the grass, a dedicated rabbit run, or a fenced section of yard — so the rabbit has the hutch for shelter and the run for movement. The two-story hutches (GUTINNEEN, Ketive, mayugardening) make good sleeping-and-shelter units because they stack usable space vertically; the long COZIWOW doubles as some of the run itself. Whatever you buy, plan the run first and let the hutch slot into it.

A hutch confined to hutch-only living has real health costs. Rabbits kept in cramped enclosures develop skeletal problems, pressure sores, and obesity, and the boredom of a tiny space drives destructive and depressed behavior. The kindest, safest setups treat the hutch as one room of a larger home — never the whole house.

## Predator-Proofing: Why You Reinforce the Mesh and Lock the Latches

Here is the single most important safety fact in this category: do not assume the wire your hutch ships with will stop a serious predator. Rabbitry and welfare guidance is consistent that half-inch welded hardware cloth is the predator-resistant standard, that the maximum safe mesh gap is 1 inch, and that chicken wire and thin galvanized mesh are inadequate — a fox or dog can tear or force them, and a raccoon can reach a paw through a 1-inch gap and injure a rabbit without ever getting inside. Where these makers document the wire — COZIWOW's galvanized mesh, Rockever's plated iron — it is better than chicken wire but is not half-inch hardware cloth. Check the wire on whichever hutch you buy, and reinforce it.

So plan to reinforce. In any yard with foxes, coyotes, loose dogs, raccoons, or birds of prey, line the vulnerable panels with half-inch hardware cloth before you trust the hutch overnight. It is inexpensive, it screws or staples on, and it is the difference between a hutch that looks secure and one that is. You can [shop half-inch hardware cloth for predator-proofing](https://www.amazon.com/s?k=1%2F2+inch+hardware+cloth+predator&tag=petpalhq08-20) and add it to whichever hutch you buy — treat it as a required accessory, not an optional upgrade.

Two more predator points matter as much as the mesh. Latches on budget hutches are usually simple slide bolts, and raccoons are notorious for learning to open them; add a carabiner, spring clip, or small padlock to every door. And open-bottom designs — the COZIWOW's grazing floor and the Rockever's floorless run — are a two-way dig route: great for foraging, dangerous unsecured. Set those on a paved base or bury a skirt of hardware cloth around and under the edge so nothing can dig in or out. Do these three things — reinforce the mesh, lock the latches, block the dig — and a hutch that started out as a target becomes a genuinely safe shelter.

## Weatherproofing and the Wood-Rot Reality

Outdoor hutches live outdoors, which means sun, rain, and freeze-thaw cycles are working against the wood from day one. The better weather features in this group are real: asphalt roofs and UV-resistant panels (the Ketive and COZIWOW) shed water and slow sun damage, cedar (the mayugardening) has a natural rot-resistance edge over plain fir, and the GUTINNEEN's aluminum-alloy reinforced frame sidesteps some of the warping that ruins all-wood boxes. Those features earn their place in the scoring.

But no wooden hutch is maintenance-free, and pretending otherwise sets you up for a rotted, moldy enclosure in two or three seasons. The joints are where water gets in and rot starts, so plan to reseal exposed wood periodically and inspect the roof edges and corners each season. In wet climates, that upkeep is the price of an outdoor wooden hutch — and if you are not willing to do it, a metal-framed hutch or an indoor pen is the more honest choice. Weatherproofing on the box only helps if you keep the box maintained.

## What We Passed On

We kept the roster to five distinct hutches, and a few common alternatives were left off on purpose.

We passed on the flood of near-identical generic wooden hutches that dominate the search results. Beyond a certain point, a 48-inch two-story fir hutch is a 48-inch two-story fir hutch, and adding three more of them would pad the list without helping you choose. We picked the Ketive as the representative best-value version of that pattern and moved on rather than list its clones.

We passed on cheap all-wire rabbit cages marketed for outdoor use. Bare wire cages with no solid shelter leave a rabbit exposed to sun, rain, and the stress of being fully visible to predators, and they fail the basic requirement that an outdoor rabbit have a covered place to hide. A hutch needs an enclosed shelter section; a naked wire box is not a hutch.

We also passed on framing any of these as a complete, buy-it-and-done solution — because none is. The most honest thing we can do is refuse to rank a "best rabbit home" when the category tops out at shelters that all need a separate run and mesh reinforcement to be safe. Pretending a $160 hutch is a finished outdoor home would sell you short and put your rabbit at risk, so we did not.

## Frequently Asked Questions

**Q: How big does an outdoor rabbit enclosure actually need to be?**

A: Bigger than any single hutch on this page. The RWAF sets a minimum footprint of 3m x 2m x 1m — about 10 by 6 by 3 feet, or roughly 65 square feet of connected floor space — for a pair of rabbits, and it must be available at all times, not just during a daily exercise hour. That is why we treat every hutch here as a shelter to attach to a larger run rather than a standalone home. Buy the hutch for sleeping and shelter, and pair it with a big exercise run so your rabbit can actually run and hop.

**Q: Are these hutches predator-proof out of the box?**

A: Not on the factory wire alone. Welfare and rabbitry guidance treat half-inch hardware cloth as the predator-resistant standard, and the wire documented here — COZIWOW's galvanized mesh, Rockever's plated iron — is not it. Thin mesh can be torn or forced by a fox or dog, and a raccoon can reach through a 1-inch gap. In any yard with predators, reinforce the panels with half-inch hardware cloth, add carabiners or padlocks to the latches, and block the base of any open-bottom design. With those additions, a factory hutch becomes genuinely secure; without them, it only looks secure.

**Q: Is chicken wire good enough for a rabbit hutch?**

A: No — this is one of the clearest points in the guidance. Chicken wire is designed to keep chickens in, not to keep predators out, and a determined dog, fox, or raccoon can tear or bend through it. Welded mesh is stronger because each joint is fixed, and half-inch hardware cloth adds the tight gap that stops a paw from reaching through. If any part of your setup uses chicken wire, replace it with half-inch welded hardware cloth before you trust it overnight.

**Q: What is the difference between a hutch and a run, and do I need both?**

A: You need both. A hutch is the enclosed, weatherproof shelter where a rabbit sleeps and hides; a run is the open, larger space where it exercises. The hutches here provide the shelter and, in the COZIWOW's case, some run space, but none provides enough run on its own to meet welfare standards. The standard safe setup is a secure hutch connected to a large run, so the rabbit can move freely between shelter and exercise space throughout the day.

**Q: How do I stop predators from digging under an open-bottom hutch?**

A: Two reliable methods. Either place the hutch on a solid base — a paved patio, concrete slab, or paving stones — so nothing can dig up into it, or bury a skirt of half-inch hardware cloth around and under the perimeter, extending it about a foot outward and downward so a digging predator hits wire before it reaches the run. The open-bottom COZIWOW and the floorless Rockever run both need one of these defenses before they are safe for unattended outdoor use.
