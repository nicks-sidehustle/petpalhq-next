...
async function getAccessToken(): Promise<string> {
  ...
  const keyJson = JSON.parse(Buffer.from(keyB64, 'base64').toString('utf-8')) as ServiceAccountKey;
...
export async function GET(request: Request) {
  try {
    ...
    return NextResponse.json({
      site: siteUrl,
      period: `${days} days`,
      topQueries: (analytics.rows as SearchAnalyticsRow[] | undefined)?.map((row) => ({
        query: row.keys[0],
        clicks: row.clicks,
      ...
      sitemaps: (sitemaps.sitemap as SitemapEntry[] | undefined)?.map((s) => ({
        path: s.path,
